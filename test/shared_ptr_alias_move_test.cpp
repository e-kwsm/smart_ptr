...
int main()
{
...

    {
        ...
        BOOST_TEST( p4.get() == nullptr );
        BOOST_TEST( p4.use_count() == 0 );
    }

    {
        boost::shared_ptr< int > p( new int );
        boost::shared_ptr< void const > p2( std::move( p ), nullptr );
...

    {
        ...
        boost::shared_ptr< incomplete > p2( std::move( p ), nullptr );
